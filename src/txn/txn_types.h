...
public:
  ...
  RMW(const std::set<Key> &writeset, double time = 0) : time_(time) {
    writeset_ = writeset;
  }
  RMW(const std::set<Key> &readset, const std::set<Key> &writeset,
      double time = 0)
      : time_(time) {
    readset_ = readset;
    writeset_ = writeset;
  ...
  virtual void Run() {
    ...
    for (std::set<Key>::iterator it = readset_.begin(); it != readset_.end();
         ++it)
      Read(*it, &result);
